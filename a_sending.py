...
def send_dataPackets():
    ...
    for packet_n in range(n_packets):
        ...
        # print( f"Control Signals:-- {axis_x}, {axis_y}, {axis_z}")
        ...
        data_packet =payload + n_fill_bytes * fill_char
        length = len(data_packet)
        payload = (data_packet).encode()
        print(payload)
        socket_out.sendall(struct.pack('!I', length))
        ...
        inter.append(sleep_time_seconds)
